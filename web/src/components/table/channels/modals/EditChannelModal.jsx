...
const EditChannelModal = (props) => {
  ...
  const applyChannelSettingsChanges = (updates) => {
    ...
    if (success) {
      ...
      if (data.setting) {
        try {
          ...
          const cachePercentageMax = Number(
            parsedSettings.cache_percentage_max ?? DEFAULT_CACHE_PERCENTAGE_MAX,
          );
          data.cache_percentage_min = Number.isFinite(cachePercentageMin)
            ? cachePercentageMin
            : DEFAULT_CACHE_PERCENTAGE_MIN;
          data.cache_percentage_max = Number.isFinite(cachePercentageMax)
            ? cachePercentageMax
  ...
  const submit = async () => {
    ...
    localInputs.cache_percentage_max = Number(
      localInputs.cache_percentage_max ??
    ...
    if (
      localInputs.cache_enabled &&
      (!Number.isInteger(localInputs.cache_percentage_min) ||
        !Number.isInteger(localInputs.cache_percentage_max) ||
        localInputs.cache_percentage_min < 0 ||
        localInputs.cache_percentage_max > 100 ||
    ...
    // 生成渠道额外设置 JSON，并保留前端尚未识别的未来字段。
    let existingChannelExtraSettings = {};
    const existingChannelSetting = inputs.setting || localInputs.setting;
    if (existingChannelSetting) {
      try {
        const parsedChannelSettings = JSON.parse(existingChannelSetting);
        if (
          parsedChannelSettings &&
          typeof parsedChannelSettings === 'object' &&
          !Array.isArray(parsedChannelSettings)
        ) {
          existingChannelExtraSettings = parsedChannelSettings;
        }
      } catch (error) {
              ...
              <div className='space-y-4'>
                ...
                <div className='py-3 border-b border-gray-100'>
                  ...
                  <Form.Switch
                    ...
                    uncheckedText={t('关')}
                    onChange={(value) =>
                      handleCacheModeChange('cache_enabled', value)
                    }
                    extraText={t(
                  ...
                  <Form.Switch
                    field='no_cache_enabled'
                    label={t('忽略上游缓存')}
                    checkedText={t('开')}
                    uncheckedText={t('关')}
                    onChange={(value) =>
                      handleCacheModeChange('no_cache_enabled', value)
                    }
                    extraText={t(
                ...
                <div className='pt-3'>
                  ...
                  <Form.InputNumber
                    ...
                    extraText={t(
                      '请求 input_tokens + max_tokens 大于等于该值时跳过此渠道，0 表示不限制',
                    )}
                  />
                  <Form.InputNumber
                    ...
                    step={1}
                    onChange={(value) =>
                      handleInputChange('max_output_tokens', Number(value) || 0)
                    }
                    extraText={t(
                      '请求 max_tokens 大于该值时跳过此渠道，0 表示不限制',
                    )}
                  />
                  <Form.InputNumber
                    field='min_input_tokens'
                    label={t('最小输入')}
                    min={0}
                    step={1}
                    onChange={(value) =>
                      handleInputChange('min_input_tokens', Number(value) || 0)
                    }
                    extraText={t(
                      '请求 input_tokens 小于该值时跳过此渠道，0 表示不限制',
                    )}
                  />
                  <Form.InputNumber
                    ...
                    step={1}
                    onChange={(value) =>
                      handleInputChange('max_input_tokens', Number(value) || 0)
                    }
                    extraText={t(
